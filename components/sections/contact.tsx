...
import { Check, ChevronLeft, ChevronRight, Mail, MapPin, Phone } from 'lucide-react';
...
type FormState = {
  ...
  budget: string; timeline: string; description: string; preferredLanguage: 'ar' | 'en'; consent: boolean;
};

const emptyForm = (locale: 'ar' | 'en'): FormState => ({ name: '', company: '', phone: '', email: '', service: '', budget: '', timeline: '', description: '', preferredLanguage: locale, consent: false });
...
export function Contact() {
  ...
  const whatsappMessage = useMemo(() => buildWhatsAppLeadMessage(locale, { name: form.name, service: serviceLabel, budget: budgetLabel, timeline: timelineLabel, summary: form.description, estimate: estimateRange, preferredLanguage: form.preferredLanguage }), [budgetLabel, estimateRange, form.description, form.name, form.preferredLanguage, locale, serviceLabel, timelineLabel]);
  ...
  const review = [[ar ? 'الاسم' : 'Name', form.name], [ar ? 'الشركة' : 'Company', form.company || '—'], [ar ? 'البريد' : 'Email', form.email], [ar ? 'الهاتف' : 'Phone', form.phone], [ar ? 'الخدمة' : 'Service', serviceLabel], [ar ? 'الميزانية' : 'Budget', budgetLabel], [ar ? 'المدة' : 'Timeline', timelineLabel], [ar ? 'لغة التواصل' : 'Preferred language', form.preferredLanguage === 'ar' ? 'العربية' : 'English']];
        ...
        <form className="mt-8" onSubmit={submit} noValidate>
          ...
          <fieldset><legend className="font-display text-xl font-semibold text-white">{labels[step]}</legend>
            ...
            {step === 2 && <div className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <label className="text-sm text-ink-secondary sm:col-span-2">{ar ? 'لغة التواصل المفضلة' : 'Preferred language'}<select className={inputClass} value={form.preferredLanguage} onChange={(e) => set('preferredLanguage', e.target.value as 'ar' | 'en')}><option value="ar">العربية</option><option value="en">English</option></select></label>
            </div>}
