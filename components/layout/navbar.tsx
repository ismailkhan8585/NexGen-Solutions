...
export function Navbar() {
  ...
  const navLinks = [
    ...
    { href: `/${locale}/blog`, label: locale === 'ar' ? 'المعرفة' : 'Insights' },
    ...
    <>
      <header
        className={cn(
          ...
          scrolled
            ? 'border-b border-surface-border bg-surface/95 shadow-[0_14px_40px_-28px_rgba(0,0,0,.9)] backdrop-blur-md'
            : 'border-b border-transparent bg-surface/20 backdrop-blur-sm'
        )}
      >
        <nav className="container-max flex h-full w-full min-w-0 items-center justify-between px-4 sm:px-6 lg:px-8">
          <Link href={`/${locale}`} className="flex min-w-0 shrink items-center gap-2" aria-label={t('common.homeLabel')}>
          ...
          <div className="hidden lg:flex items-center gap-6">
            {navLinks.map((link) => {
              ...
              <Link
                ...
                className={cn('group relative text-sm font-medium transition-colors', active ? 'text-white' : 'text-ink-secondary hover:text-white')}
              >
                {link.label}
                <span className={cn('absolute -bottom-2 start-0 h-0.5 bg-brand-cyan-400 transition-all duration-300', active ? 'w-full' : 'w-0 group-hover:w-full')} />
              </Link>
            )})}
          </div>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-[60] lg:hidden">
          ...
          />
          <div id="mobile-navigation" role="dialog" aria-modal="true" aria-label={locale === 'ar' ? 'التنقل الرئيسي' : 'Main navigation'} className="mobile-menu-panel absolute right-0 top-0 bottom-0 w-full max-w-sm bg-surface border-l border-surface-border p-6 flex flex-col shadow-2xl shadow-black/40 rtl:left-0 rtl:right-auto rtl:border-l-0 rtl:border-r">
            ...
            <div className="flex flex-col gap-1">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={cn('min-h-[48px] rounded-xl px-4 py-3 font-medium transition-colors', pathname === link.href || pathname.startsWith(`${link.href}/`) ? 'bg-surface-hover text-white' : 'text-ink-secondary hover:bg-surface-hover hover:text-white')}
                >
                  {link.label}
                </Link>
              ))}
