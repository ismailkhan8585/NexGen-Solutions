...
export async function GET() {
  ...
  const [inquiries, blogPosts] = await Promise.all([
    prisma.projectInquiry.findMany({
      select: { service: true, company: true, status: true },
    }),
    prisma.blogPost.findMany({
      where: { isPublished: true },
      select: { titleEn: true, views: true },
    }),
  ]);
  ...
  inquiries.forEach((inq) => {
    ...
    if (inq.company)
      byCountryMap.set(inq.company, (byCountryMap.get(inq.company) ?? 0) + 1);
    byStatusMap.set(inq.status, (byStatusMap.get(inq.status) ?? 0) + 1);
  });

  return NextResponse.json({
    totalInquiries: inquiries.length,
    convertedInquiries: inquiries.filter((i) => i.status === "CONVERTED")
      .length,
    ...
    blogViews: blogPosts
      .map((p) => ({ title: p.titleEn, views: p.views }))
