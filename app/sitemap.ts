...
const staticPaths=['','about','services','industries','support','work','blog','team','pricing','contact','privacy-policy','terms-and-conditions','cookie-policy','refund-cancellation-policy'];
export default async function sitemap():Promise<MetadataRoute.Sitemap>{
  const base=businessConfig.appUrl.replace(/\/$/,''); const now=new Date();
  const paths=[...staticPaths,...serviceCatalog.map(s=>`services/${s.slug}`),...industrySolutions.map(s=>`industries/${s.slug}`),...seoLandings.map(s=>`solutions/${s.slug}`)];
